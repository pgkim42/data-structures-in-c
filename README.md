...
## Chapter 13: 탐색 (Search)

### 기본 탐색 알고리즘

- **binary_search.c**: 이진 탐색 (Binary Search)
  - 정렬된 배열에서 O(log n) 시간 복잡도로 탐색
  - 반복문 버전과 재귀 버전 구현
  - 오버플로우 방지: `mid = low + (high - low) / 2`
  - 탐색 과정 시각화 (범위, mid, 비교 결과)
  - 시간 복잡도: O(log n), 공간 복잡도: O(1)

- **indexed_sequential_search.c**: 색인 순차 탐색 (Indexed Sequential Search)
  - 정렬된 대용량 배열에서 색인 테이블 활용
  - IndexEntry 구조체 (key, start)로 구간 정보 저장
  - 2단계 탐색: ① 색인 테이블로 구간 찾기 → ② 구간 내 순차 탐색
  - 디스크 파일처럼 임의 접근 비용이 높은 환경에서 효율적
  - 시간 복잡도: O(n/m + m) = O(n), 공간 복잡도: O(m)

- **interpolation_search.c**: 보간 탐색 (Interpolation Search)
  - 정렬된 배열에서 값의 비례 관계를 이용해 위치 예측
  - 보간 공식: `pos = low + ((target - arr[low]) × (high - low)) / (arr[high] - arr[low])`
  - 균등 분포 데이터에서 O(log log n) - 이진 탐색보다 빠름
  - 불균형 분포 데이터에서는 O(n)으로 성능 저하
  - 반복문 버전과 재귀 버전 구현
  - 균등 분포 vs 불균형 분포 비교 테스트

### 탐색 알고리즘 비교

| 탐색 방법 | 시간 복잡도 | 공간 복잡도 | 조건 | 특징 |
|-----------|-------------|-------------|------|------|
| 순차 탐색 | O(n) | O(1) | 없음 | 단순하지만 느림 |
| 이진 탐색 | O(log n) | O(1) | 정렬 필요 | 가장 널리 사용됨 |
| 색인 순차 | O(n/m + m) | O(m) | 정렬 + 색인 필요 | 대용량 데이터/디스크 I/O에 유리 |
| 보간 탐색 | O(log log n) 평균 | O(1) | 정렬 + 균등 분포 | 균등 데이터에서 최고, 불균형에서 느림 |
